...
class Time extends DateTime
{
    ...
    public function humanize()
    {
        ...
        if ($years !== 0) {
            $phrase = $this->lang('Time.years', [abs($years)]);
            $before = $years < 0;
        } else if ($months !== 0) {
            $phrase = $this->lang('Time.months', [abs($months)]);
            $before = $months < 0;
        } else if ($days !== 0 && (abs($days) >= 7)) {
            $weeks = ceil($days / 7);
            $phrase = $this->lang('Time.weeks', [abs($weeks)]);
            $before = $days < 0;
        } else if ($days !== 0) {
            $before = $days < 0;
            ...
            if (abs($days) === 1) {
                return $before ? $this->lang('Time.yesterday') : $this->lang('Time.tomorrow');
            }

            $phrase = $this->lang('Time.days', [abs($days)]);
        } else if ($hours !== 0) {
            // Display the actual time instead of a regular phrase.
            return $this->format('g:i a');
        } else if ($minutes !== 0) {
            $phrase = $this->lang('Time.minutes', [abs($minutes)]);
            $before = $minutes < 0;
        } else {
            return $this->lang('Time.now');
        }

        return $before ? $this->lang('Time.ago', [$phrase]) : $this->lang('Time.inFuture', [$phrase]);
    }
    ...
    private function lang(string $keys, array $ar = []): string|false {
        $key0 = explode('.', $keys)[0];
        $key1 = explode('.', $keys)[1];
        $array = [
            'Time' => [
        ...
        return $array[$key0][$key1] ?? ($key1 ?? false);
    }
